...
const aiInsights = [
  {
    ...
    description:
      'Your vitamin D levels have been consistently low across multiple reports. Consider increasing sun exposure and supplementation.',
  ...
  },
  {
    ...
    description:
      'Your lipid profile shows excellent HDL levels, indicating good cardiovascular health maintenance.',
            ...
            <div className='mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left'>
              ...
              <div className='mt-2'>
                <p className='text-sm text-gray-500'>
                  Your recent blood pressure readings show concerning patterns that require
                  immediate medical attention. Please consult with a physician within 24-48 hours.
                </p>
...
export default function HealthReportsPage() {
  ...
  const filteredReports = recentReports.filter(report => {
    const matchesCategory =
      selectedCategory === 'all' || report.type.toLowerCase().includes(selectedCategory);
    const matchesSearch =
    ...
    <div className='p-6 lg:p-8'>
      ...
      <div className='mb-8'>
        <div className='flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold text-gray-900 font-display mb-2'>Health Reports</h1>
            ...
            <div className='relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md sm:p-6'>
              <div className='text-center'>
                ...
                <p className='text-sm text-gray-600 mb-6'>
                  Upload PDF files, images, or scan reports directly. Our AI will analyze and
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8'>
        {aiInsights.map((insight, index) => (
          <div key={index} className='bg-white rounded-xl shadow-sm border border-gray-200 p-6'>
            ...
            <div className='text-xs text-gray-500'>Based on: {insight.reports.join(', ')}</div>
          </div>
        ))}
      </div>
              ...
              <div className='mb-4'>
                ...
                <div className='space-y-2'>
                  {report.insights.map((insight, index) => (
                    <div
                      key={index}
                      className={`p-2 rounded-lg border text-sm ${getInsightColor(insight.type)}`}
                    >
                      {insight.text}
                    </div>
                  ))}
            ...
            {Object.keys(report.keyMetrics).length > 0 && (
              <div>
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-3'>
                  {Object.entries(report.keyMetrics).map(
                    ([key, metric]: [
                      string,
                      { value: string; unit: string; range: string; status: string },
                    ]) => (
                      <div key={key} className='bg-gray-50 rounded-lg p-3'>
                            ...
                            className={`px-2 py-1 rounded-full text-xs font-medium ${
                              metric.status === 'normal'
                                ...
                                : metric.status === 'good'
                                  ...
                                  : metric.status === 'low' || metric.status === 'elevated'
                                    ? 'bg-yellow-100 text-yellow-800'
                                    : 'bg-red-100 text-red-800'
                            }`}
                          >
                        ...
                        <div className='text-xs text-gray-500 mt-1'>Normal: {metric.range}</div>
                      </div>
                    )
                  )}
                </div>
